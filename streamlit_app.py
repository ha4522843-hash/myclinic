...
if (user_role == "الجراح (الدكتورة)" and password == "111") or (user_role == "السكرتيرة" and password == "222"):
    ...
    if sheet:
        ...
        if user_role == "السكرتيرة":
            ...
            with st.form("medical_form", clear_on_submit=True):
                ...
                if submit and name:
                    ...
                    row = [datetime.now().strftime("%Y-%m-%d %H:%M"), name, str(dob), str(final_age), job, social, final_source, check_type, final_chronic, final_surgery, notes]
        ...
        # 4. واجهة الدكتورة
            ...
            if len(all_data) > 1:
                # تحويل البيانات لجدول وترتيبها (الأحدث فوقر
