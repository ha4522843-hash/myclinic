    ...
    if sheet:
        ...
        if user_role == "السكرتيرة":
            ...
            with tab_register:
                ...
                search_q = st.text_input("🔍 ابحث عن اسم المريض لاستدعاء بياناته:")
                p_found = None
                if search_q and not df_main.empty:
                    matches = df_main[df_main['الاسم'].str.contains(search_q, na=False)]
                    if not matches.empty:
                        p_found = matches.iloc[-1]
                        st.success(f"✅ تم استدعاء بيانات: {p_found['الاسم']}")
